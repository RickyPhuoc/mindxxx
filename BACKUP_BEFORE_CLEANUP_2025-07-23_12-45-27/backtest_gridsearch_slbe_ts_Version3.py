...
def simulate_trade(pair, df_candle, sl, be, ts_trig, ts_step):
    ...
    # Debug info for first trade
    if pair['num'] == 1:
        print(f"🔍 DEBUG Trade #1:")
        print(f"   Entry DT: {pair['entryDt']} (type: {type(pair['entryDt'])})")
        print(f"   Exit DT: {pair['exitDt']} (type: {type(pair['exitDt'])})")
        print(f"   Candle time range: {df_candle['time'].min()} → {df_candle['time'].max()}")
        print(f"   Total candles: {len(df_candle)}")
    ...
    # Entry: Execute at next candle OPEN (standard practice)
    # Exit: Execute at signal candle CLOSE (more realistic than next open)
    entryPrice = prices.iloc[0]['open']  # Entry tại open của nến entry
    exitPrice = prices.iloc[-1]['close']  # Exit tại close của nến exit
