...
import pandas as pd
import numpy as np
from datetime import datetime, ti    # ===== TRADELIST VALIDATION =====
    ...
    print(f"   Match: {'✅' if abs(actual_pnl - calculated_pnl) < 0.01 else '❌'}")ef deep_analyze_trade_139():
    ...
    actual_exit_price = float(trade_data['Exit Price'])
    actual_pnl = float(trade_data['PnL %'])
    calculated_pnl = (entry_price - actual_exit_price) / entry_price * 100
